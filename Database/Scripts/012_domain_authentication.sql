...
BEGIN TRY
    ...
    -- Script 013 removes these transitional columns. Dynamic SQL keeps this migration safe to rerun afterward.
    IF COL_LENGTH(N'dbo.auth_UserAccount', N'DomainAccount') IS NOT NULL
       AND COL_LENGTH(N'dbo.auth_UserAccount', N'AuthenticationType') IS NOT NULL
        EXEC sys.sp_executesql N'
            ...
            CREATE UNIQUE INDEX UX_auth_UserAccount_DomainAccount ON dbo.auth_UserAccount(DomainAccount);
            ALTER TABLE dbo.auth_UserAccount WITH CHECK ADD CONSTRAINT CK_auth_UserAccount_DomainOnly CHECK (AuthenticationType = ''WINDOWS'');
